...
function EditPage() {
  ...
  const handleComprobarContrasenia = (event) => {
    ...
    fetch(`${API_URL}/validar-contrasena`, {
      ...
      body: JSON.stringify({ id_usuario: parseInt(usuarioActivo.id_usuario), contraseña: password })
    })
      .then(response => response.json())
      .then(data => {
      ...
      <div className="flex flex-col justify-center items-center">
        <Card>
          ...
          >
            ...
            <Label htmlFor="password"> Confirme contraseña para guardar cambios:</Label>
            <Input
              type="password"
              name="password"
            ...
            />
            <button className="bg-green-500 px-4 py-1 rounded-md my-2 disabled:bg-indigo-300" onClick={handleComprobarContrasenia}> Comprobar Contraseña</button>
            <br/>
            <button className="bg-indigo-500 px-4 py-1 rounded-md my-2 disabled:bg-indigo-300" onSubmit={onSubmit}>Editar Información</button>
          </form>
        </Card>
      </div>
    </div>
